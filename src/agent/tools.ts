...
2. 「如何实现 / 怎么生成 / 源码逻辑」类问题：优先一次调用 analyze_project，不要反复 search
3. 可少量串联工具（如先 list 再 read），但避免重复相同搜索；信息足够后立即停止
4. 纯闲聊、通用知识、与本地工作区无关时，不调用任何工具
5. 路径一律使用相对工作区根目录的路径；列出根目录项目时 relativePath 传 ""
6. 不要编造工具结果；不要输出 [TOOL_CALL]、function_calls 等标记`;

export const AGENT_MAX_ROUNDS = 4;
export const AGENT_MAX_TOOL_CALLS = 6;
/** @deprecated 使用 AGENT_MAX_ROUNDS */
export const AGENT_MAX_STEPS = AGENT_MAX_ROUNDS;
